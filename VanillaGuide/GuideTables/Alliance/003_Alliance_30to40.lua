...
-----------31-31 Alterac Mountains
	...
	[3131] = {
		...
		items = {
			...
			[2] = { str = "01) Run up into Alterac Mountains near the AV entrance around 64,38 and kill lions for #DOQUESTCostly Menace# They’re all along the southern border near AV. I usually get all mine in front and West of AV", x = 64, y = 38, zone = "Alterac Mountains" },
			[3] = { str = "02) Go east of AV and do  #DOQUESTSyndicate Assassins#  at the camp around 55,67. There is a camp close to AV but this one has a quest you can grab near the tents on the table. Clear the camp and touch the syndicate document, accept  #ACCEPTForeboding Plans#  and  #ACCEPTEncrypted Letter#", x = 55, y = 67, zone = "Alterac Mountains" },
			[4] = { str = "03) #PICTUREHearth# to Southshore,#HUNTERstable pet, go to Alterac and tame a level 32 Mountain Lion ( the ones you killed before) for Prowl rank 1 (stealth)#" },
			[5] = { str = "#HUNTER04) Die on purpose so you end up in Southshore and then get your cat back out#"},
			[6] = { str = "05) Go into the town hall, turn in  #TURNINSyndicate Assassins#  and  #TURNINForeboding Plans#  accept  #ACCEPTNoble Deaths#" },
			[7] = { str = "06) Stop at the house North of the Inn, turn in  #TURNINEncrypted Letter#  accept  #ACCEPTLetter to Stormpike#" },
			[8] = { str = "07) Go to the barn and turn in  #TURNINCostly Menace#" },
			[9] = { str = "08) #PICUTREFly to Refuge Point#" },
		}
	},
...
-----------32-32 Stranglethorn Vale
	...
	[3232] = {
		...
		items = {
			...
			[2] = { str = "01) Run to the Rebel Camp at 37,3 its just west after you enter.", x = 37, y = 3, zone = "Stranglethorn Vale" },
			[3] = { str = "02) Accept  #ACCEPTThe Second Rebellion#  and  #ACCEPTBad Medicine#" },
			[4] = { str = "03) Then accept  #ACCEPTKrazek's Cookery#" },
			[5] = { str = "04) Go to Nessingway’s at 35,10 and accept  #ACCEPTWelcome to the Jungle#  then turn it in behind you. This opens the 3 quest givers here.", x = 35, y = 10, zone = "Stranglethorn Vale" },
			[6] = { str = "05) Now Accept  #ACCEPTRaptor Mastery pt.1#   #ACCEPTPanther Mastery pt.1#   #ACCEPTTiger Mastery pt.1#  Don’t accept the page-collecting quest until you have all the pages." },
			[7] = { str = "#HUNTER06) Go to 29,8 just West of Nessingway’s and tame a Stranglethorn Tiger for Dash Rank 1. Use this pet until you get to Booty Bay#", x = 29, y = 8, zone = "Stranglethorn Vale"},
			[8] = { str = "07) Do  #DOQUESTPanther Mastery pt.1#  &  #DOQUESTTiger Mastery pt.1#  Tigers are just behind Nessingway’s, and the Panthers are to the east some but centralized around 41,9", x = 41, y = 9, zone = "Stranglethorn Vale" },
			[9] = { str = "08) Go to 44,10 and do  #DOQUESTThe Second Rebellion#  and the Kurzen supply crate at 44,9 in front of the Inn for  #DOQUESTBad Medicine#  the remedies are a low drop from medicine men.", x = 44, y = 10, zone = "Stranglethorn Vale" },
			[10] = { str = "09) Run back up to the rebel camp at 37,3 Turn in  #TURNINThe Second Rebellion#  and  #TURNINBad Medicine#  SKIP  #VIDEOSpecial Forces#.", x = 37, y = 3, zone = "Stranglethorn Vale" },
			[11] = { str = "10) Go back to Nessingway’s at 35,10 and hand in  #TURNINPanther Mastery# pt.1  &  #TURNINTiger Mastery pt.1#  accept  #ACCEPTPanther Mastery pt.2#  &  #ACCEPTTiger Mastery pt.2#.", x = 35, y = 10, zone = "Stranglethorn Vale" },
			[12] = { str = "11) Kill Stranglethorn Tigers for around 29,10 for  #DOQUESTTiger Mastery pt.2# and the Panthers around 28,12 for  #DOQUESTPanther Mastery pt.2#.", x = 29, y = 10, zone = "Stranglethorn Vale" },
			[13] = { str = "12) Kill Stranglethorn Raptors for 26,15  #DOQUESTRaptor Mastery pt.1#", x = 26, y = 15, zone = "Stranglethorn Vale" },
			[14] = { str = "13) Go back to Nessingway’s at 35,10 and turn in  #TURNINTiger Mastery pt.2#,  #TURNINPanther Mastery pt.2#, and  #TURNINRaptor Mastery pt.1#. Accept  #ACCEPTTiger Mastery pt.3#,  #ACCEPTPanther Mastery pt.3#  and  #ACCEPTRaptor Mastery pt.2#", x = 35, y = 10, zone = "Stranglethorn Vale" },
			[15] = { str = "14) Run to Booty Bay and get the FP." },
			[16] = { str = "15) Turn in  #TURNINKrazek's Cookery#  on the top floor of the inn, accept and hand back in (the bloodstone ores)  #ACCEPTFavor for Krazek#  then accept  #ACCEPTReturn to Corporal Kaleb#  (this quest gives you your first helm) also accept  #ACCEPTInvestigate the Camp#  and  #ACCEPTSupplies to Private Thorsen#" },
			[17] = { str = "16) Get on the Boat to Rachet" },
			[18] = { str = "17) #PICTUREGrab the FP#" },
			[19] = { str = "18) Now for a little bit of a run through horde territory. Run all the way south in the barrens to the great lift and take it down into 1k needles. Jump off if you get attacked." },
			[20] = { str = "19) Run all the way south in the barrens to the great lift and take it down into 1k needles. Jump off if you get attacked." },
		}
	},
...
-----------33-35 Desolace
	...
	[3335] = {
		...
		items = {
			...
			[2] = { str = "01) Make Nijel’s Point your home then fly back to Stonetalon and hand in  #TURNINReclaiming the Charred Vale#  SKIP the next part." },
			[3] = { str = "02) #PICTUREHearth# back to Desolace" },
			[4] = { str = "03) Go south of the Inn, hand in  #TURNINReclaimers' Business# in Desolace  accept  #ACCEPTReagents for Reclaimers Inc pt.1#  and  #ACCEPTThe Karnitol Shipwreck pt.1#" },
			[5] = { str = "04) I SKIP both of the faction centaur quests. It’s just a useless grind for 2 quests that aren’t worth it." },
			[6] = { str = "05) Accept  #ACCEPTCentaur Bounty#" },
			[7] = { str = "06) Go south a little bit more and accept  #ACCEPTVahlarriel's Search pt.1#" },
			[8] = { str = "07) Run west to 56,17  #DOQUESTVahlarriel's Search pt.1#  accept  #ACCEPTVahlarriel’s Search pt.2#", x = 56, y = 17, zone = "Desolace" },
			[9] = { str = "08) Run east to Sargeron around 75,20 and kill the demons for  #DOQUESTReagents for Reclaimers Inc pt.1#", x = 75, y = 20, zone = "Desolace" },
			[10] = { str = "09) Run back to Nijel’s, hand in  #TURNINVahlarriel’s Search pt.2#  accept  #ACCEPTVahlarriel’s Search pt.3#" },
			[11] = { str = "10) Run up a bit more, turn in  #TURNINReagents for Reclaimers Inc pt.1#  accept  #ACCEPTReagents for Reclaimers Inc pt.2#" },
			[12] = { str = "11) Now start grinding on the scorps and other stuff for  #DOQUESTReagents for Reclaimers Inc pt.2#  as you head to 62,38 and accept  #ACCEPTBone Collector#  since the scorps are a low drop rate just kill them as you’re running around", x = 62, y = 38, zone = "Desolace" },
			[13] = { str = "12) Run east until you’re at kolkar village around 72,45. Kill them for  #DOQUESTCentaur Bounty#  If you’re not 34 when done with this, grind until you are", x = 72, y = 45, zone = "Desolace" },
			[14] = { str = "13) Grind down to 60,61 and accept  #ACCEPTKodo Roundup#  This is a unique quest where you tame a kodo at the GY then bring it back until you have 5.", x = 60, y = 61, zone = "Desolace" },
			[15] = { str = "14) If you see the pack of aged kodo’s running around kill 3 of them for  #DOQUESTReagents for Reclaimers Inc pt.2#  if not you can find some inside the kodo graveyard." },
			[16] = { str = "15) Go to the Kodo Graveyard around 51,58. Do  #DOQUESTBone Collector#  everytime you come to pick up a kodo for roundup.", x = 51, y = 58, zone = "Desolace" },
			[17] = { str = "16) Once you’re done with  #DOQUESTKodo Roundup#  hand it in and run NW of the GY towards the water. Killing scorps if you still need them." },
			[18] = { str = "17) Grab  #ACCEPTSceptre of Light#  at 38,27 near the big tower", x = 38, y = 27, zone = "Desolace" },
			[19] = { str = "18) Go south on the beach to 36,30 and finish  #DOQUESTThe Karnitol Shipwreck pt.1#  accept  #ACCEPTThe Karnitol Shipwreck pt.2#  then accept  #ACCEPTClaim Rackmore's Treasure!#  from Rackmore’s log. If your quest log is full drop centaur bounty.", x = 36, y = 30, zone = "Desolace" },
			[20] = { str = "19) Kill Drysnaps in the waters right here for the silver key of  #DOQUESTClaim Rackmore's Treasure!#  and The Slitherblades just North of here for the golden key." },
			[21] = { str = "20) Run up to Thunder Axe Fortress at 56,29 and go in the main building in the center and turn in  #TURNINVahlarriel’s Search pt.3#  at 54,26 accept  #ACCEPTSearch for Tyranis#", x = 54, y = 26, zone = "Desolace" },
			[22] = { str = "21) Run out and right, kill a seer up in a tower for  #DOQUESTSceptre of Light#" },
			[23] = { str = "22) Go to the building west of the entrance at 53,29 kill Tyranis Malem for  #DOQUESTSearch for Tyranis#  then turn it back in inside the main building. Clear the way in the main building to the right because an escort is next.", x = 53, y = 29, zone = "Desolace" },
			[24] = { str = "23) Accept the escort quest  #ACCEPTReturn to Vahlarriel#  DON’T attack anything unless it hits or or she’ll ignore it and keep walking. The reason it seems is melee just follow her, only the mages can hit her to start combat." },
			[25] = { str = "24) Go to 62,38 and turn in  #TURNINBone Collector#", x = 62, y = 38, zone = "Desolace" },
			[26] = { str = "25) Run back to 38,27 and turn in  #TURNINSceptre of Light#  accept  #ACCEPTBook of the Ancients#", x = 38, y = 27, zone = "Desolace" },
			[27] = { str = "26) Swim all the way NW on the map to Ranazjar Isle and open the chest at 30,9 for  #DOQUESTClaim Rackmore's Treasure!#", x = 30, y = 9, zone = "Desolace" },
			[28] = { str = "27) Go to the back of the island and clear all around the serpent statue at 28,7. You’re gonna do  #DOQUESTBook of the Ancients#  This quest bugs if you do not click the gem in your inventory and then on the statue. Don’t just click on the statue. He’s 38 but super easy.", x = 28, y = 7, zone = "Desolace" },
			[29] = { str = "28) Swim back to 38,27 and turn in  #TURNINBook of the Ancients#", x = 38, y = 27, zone = "Desolace" },
			[30] = { str = "29) #PICTUREHearth# to Nijel’s" },
			[31] = { str = "30) Go south of the Inn and turn in  #TURNINReagents for Reclaimers Inc pt.2#  SKIP pt.3 for now, and  #ACCEPTThe Karnitol Shipwreck pt.2#  SKIP the rest." },
			[32] = { str = "31) Go south a bit more, turn in  #TURNINCentaur Bounty#" },
			[33] = { str = "32) Finally turn in  #TURNINReturn to Vahlarriel#" },
			[34] = { str = "#HUNTERS33)table cat, Go around the Kodo GY and tame a lvl 34 scorp for claw rank 5. Or tame one around 37,80#", x = 37, y = 80, zone = "Desolace" },
			[35] = { str = "34) You should be within 2 bars to level 35 now. I grind down on the Centaurs at Gelkis Village at 37,80 until level 35", x = 37, y = 80, zone = "Desolace" },
			[36] = { str = "35) Head south into Feralas to get the FP, this will again save more time later on." },
			[37] = { str = "36) Get on the Boat at 43,42 and grab the FP in Feathermoon. Fly to Theramore, then to Rachet.", x = 43, y = 42, zone = "Desolace" },
			[38] = { str = "37) Go to the engineer hut at 62,36 and hand in  #TURNINGoblin Sponsorship pt.1#  accept  #ACCEPTGoblin Sponsorship pt.2#  Then run out on the dock and hand in  #TURNINWharfmaster Dizzywig#  accept  #ACCEPTParts for Kravel#", x = 62, y = 36, zone = "Desolace" },
			[39] = { str = "38) Get on the boat to Booty Bay" },
		}
	},
...
-----------37-38 Arathi Highlands
	...
	[3738] = {
	    ...
	    items = {
			...
			[6] = { str = "Go to the circle of east binding, just west of Hammerfall at 66,30 and grab the cresting key from the rock.", x = 66, y = 30, zone = "Arathi Highlands" },
